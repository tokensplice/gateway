...
// MaxTokenGroupNameLength bounds the user-defined grouping label. It matches
// the varchar(50) column so a stored group name never gets truncated.
const MaxTokenGroupNameLength = 50
...
type Token struct {
	...
	// RateLimitRPM caps requests per minute for this token. Zero falls back to
	// the deployment-wide default (TOKEN_DEFAULT_RATE_LIMIT_RPM); a negative
	// value is rejected by the API.
	RateLimitRPM int `json:"rate_limit_rpm" gorm:"default:0"`
	// RateLimitTPM caps prompt+completion tokens per minute. Zero means unlimited.
	RateLimitTPM int `json:"rate_limit_tpm" gorm:"default:0"`
	// DailySpendingCap / MonthlySpendingCap cap consumed quota per UTC day and
	// per UTC month. Zero means unlimited.
	DailySpendingCap   int64          `json:"daily_spending_cap" gorm:"bigint;default:0"`
	MonthlySpendingCap int64          `json:"monthly_spending_cap" gorm:"bigint;default:0"`
	GroupName          string         `json:"group_name" gorm:"type:varchar(50);default:'';index"`
	DeletedAt          gorm.DeletedAt `gorm:"index"`
}

// ExpirationWarningDays is how far ahead of expiry a token starts surfacing a
// warning in API responses.
const ExpirationWarningDays = 7

// ExpirationWarning returns the rounded-up number of days until expiry when the
// token expires within ExpirationWarningDays, and zero otherwise. Never-expiring
// tokens (-1) and already expired tokens report no warning.
func (token *Token) ExpirationWarning(now int64) int64 {
	if token.ExpiredTime <= 0 {
		return 0
	}
	remaining := token.ExpiredTime - now
	if remaining <= 0 || remaining > ExpirationWarningDays*24*3600 {
		return 0
	}
...
// GetAllUserTokens lists a user's tokens, optionally restricted to one
// user-defined group label. It also returns the number of matching tokens so
// callers can paginate without a second query.
func GetAllUserTokens(userId int, groupName string, startIdx int, num int) ([]*Token, int64, error) {
	var tokens []*Token
...
// GetUserTokenGroups returns the distinct group labels a user has assigned to
// their tokens, sorted for a stable dropdown order.
func GetUserTokenGroups(userId int) ([]string, error) {
	var groupNames []string
	err := DB.Model(&Token{}).
		Where("user_id = ? AND group_name <> ''", userId).
		Select("DISTINCT group_name").
		Order("group_name").
		Find(&groupNames).Error
...
func (token *Token) Update() (err error) {
	...
	return DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		...
		"rate_limit_rpm", "rate_limit_tpm", "daily_spending_cap", "monthly_spending_cap", "group_name").Updates(token).Error
}
